...
void process(WifiData client) {
  ...
  if (action == "custom") {
    ...
    int start = 0;
    if (client.read() == '/') {
      start = client.parseInt();
    }

    int count = NUM_LEDS;
    if (client.read() == '/') {
      count = client.parseInt();
    }
...
void setData(WifiData client, int count) {
...
void getData(WifiData client, int count) {

  // Send feedback to client
  client.println("Status: 200 OK");
  for ( int i = 0; i < count; i++) {
    client.print(leds[i].r);
    client.print(F(" "));
    client.print(leds[i].g);
    client.print(F(" "));
    client.println(leds[i].b);
  }
  client.print(EOL);    //char terminator
}
